...
export default async function RequestDetailPage({ params }: PageProps<"/requests/[id]">) {
  ...
  if (!data) return <AppShell><PageHeader title="ไม่สามารถเปิดคำขอได้" description="ฐานข้อมูลยังไม่พร้อมใช้งาน"/><div role="alert" className="mt-5 border border-red-300 bg-[var(--red-soft)] p-4 text-[var(--red)]">{error}</div></AppShell>;
