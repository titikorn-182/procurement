...
begin
  ...
  if current_profile.id is null then
    raise exception 'ไม่พบโปรไฟล์ผู้ใช้งานหรือบัญชีถูกระงับ';
  end if;
  if current_profile.department_id is null then
    raise exception 'โปรไฟล์ยังไม่ได้กำหนดหน่วยงาน';
  end if;
  ...
  insert into public.procurement_requests (
    ...
    budget_year, fund_source, plan_name, expense_category, estimated_amount,
    ...
    nullif(trim(request_expense_category), ''), item_total,
    'submitted', 2, now()
  ) returning * into new_request;

  for item in select * from jsonb_array_elements(request_items)
  loop
    insert into public.request_items (request_id, line_no, description, quantity, unit, unit_price)
    values (
  ...
  values (new_request.id, current_profile.id, 'submit', 'ยื่นคำขอเข้าสู่กระบวนการ');

  return query select new_request.id, new_request.request_no;
end;
$$;

revoke all on function public.submit_procurement_request(public.request_kind,text,text,date,integer,text,text,text,jsonb) from public;
grant execute on function public.submit_procurement_request(public.request_kind,text,text,date,integer,text,text,text,jsonb) to authenticated;
